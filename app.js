...
app.get("/chart/:timestamp", async (req, res) => { try { const timestamp = req.params.timestamp; if (!timestamp || isNaN(timestamp)) { return res.status(400).json({ error: "Invalid timestamp parameter" }); } const intervalInMinutes = 1; const startTimeInMillis = Number(timestamp); const endTimeInMillis = Date.now(); const growwUrl = `https://groww.in/v1/api/charting_service/v2/chart/delayed/exchange/NSE/segment/CASH/NIFTY?endTimeInMillis=${endTimeInMillis}&intervalInMinutes=${intervalInMinutes}&startTimeInMillis=${startTimeInMillis}`; const response = await fetch(growwUrl); if (!response.ok) { throw new Error(`Groww API error: ${response.status}`); } const data = await response.json(); res.json(data); } catch (err) { res.status(500).json({ error: "Failed to fetch chart data", details: err.message }); } });
